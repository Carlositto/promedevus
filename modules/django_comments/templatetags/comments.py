...
class BaseCommentNode(template.Node):
    ...
    def get_queryset(self, context):
        ...
        if 'sort_by' in context:
            reversed_main = self.sort_comments(qs, context['sort_by'])
            # Te dwie linijki poniżej są do "przeciążenia" domyślnego sortowania z meta
            preserved = Case(*[When(id=id, then=pos) for pos, id in enumerate(reversed_main)])
            qs = qs.filter(id__in=reversed_main).order_by(preserved)

        return qs

    def sort_comments(self, qs, sort):
        final_tree = []
        root_comments = qs.filter(parent_id__isnull=True).order_by(sort)
        for comment in root_comments:
            final_tree.append(comment.id)
            self.append_and_sort_children(final_tree, qs, comment)
        return final_tree

    def append_and_sort_children(self, final_tree, tree, parent):
        for comment in parent.children.all():
            final_tree.append(comment.id)
